...
const controlCloroController = {
  ...
  getFormsByUserIdAndStatus: async (req, res) => {
    try {
      ...
      const forms = await ControlCloro.find({ idUser: userId, status: { $ne: "" } });
  ...
  editFormProperties: async (req, res) => {
    try {
      ...
      const updatedForm = await ControlCloro.findByIdAndUpdate(
        formId,
      ...
      const cargaIndex = user.controlcloro.findIndex((c) => c._id.toString() === formId);
      if (cargaIndex !== -1) {
        if (status !== undefined) {
          user.controlcloro[cargaIndex].status = status;
        }
        if (motivo !== undefined) {
          user.controlcloro[cargaIndex].motivo = motivo;
        }
        if (motivoRespuesta !== undefined) {
          user.controlcloro[cargaIndex].motivoRespuesta = motivoRespuesta;
        }
        if (motivoPeticion !== undefined) {
          user.controlcloro[cargaIndex].motivoPeticion = motivoPeticion;
        }
        if (typeof editEnabled === 'boolean') {
          user.controlcloro[cargaIndex].editEnabled = editEnabled;
        }
        if (whoApproved !== undefined) {
          user.controlcloro[cargaIndex].whoApproved = whoApproved;
        }
  ...
  editFormById: async (req, res) => {
    try {
      ...
      const existingForm = await ControlCloro.findById(formId);
      ...
      const updatedForm = await ControlCloro.findByIdAndUpdate(formId, formData, { new: true });
      ...
      const formIndex = user.controlcloro.indexOf(formId);

      // Reemplazar el formulario antiguo con el formulario actualizado
      if (formIndex !== -1) {
        user.controlcloro.splice(formIndex, 1, updatedForm._id);
        await user.save();
      }
