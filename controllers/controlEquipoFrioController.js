...
const controlEquipoFrioController = {
  ...
  getFormsByUserIdAndStatus: async (req, res) => {
    try {
      ...
      const forms = await ControlEquipoFrio.find({ idUser: userId, status: { $ne: "" } });
  ...
  editFormProperties: async (req, res) => {
    try {
      ...
      const updatedForm = await ControlEquipoFrio.findByIdAndUpdate(
        formId,
      ...
      const cargaIndex = user.controlequipofrio.findIndex((c) => c._id.toString() === formId);
      if (cargaIndex !== -1) {
        if (status !== undefined) {
          user.controlequipofrio[cargaIndex].status = status;
        }
        if (motivo !== undefined) {
          user.controlequipofrio[cargaIndex].motivo = motivo;
        }
        if (motivoRespuesta !== undefined) {
          user.controlequipofrio[cargaIndex].motivoRespuesta = motivoRespuesta;
        }
        if (motivoPeticion !== undefined) {
          user.controlequipofrio[cargaIndex].motivoPeticion = motivoPeticion;
        }
        if (typeof editEnabled === 'boolean') {
          user.controlequipofrio[cargaIndex].editEnabled = editEnabled;
        }
        if (whoApproved !== undefined) {
          user.controlequipofrio[cargaIndex].whoApproved = whoApproved;
        }
  ...
  editFormById: async (req, res) => {
    try {
      ...
      const existingForm = await ControlEquipoFrio.findById(formId);
      ...
      const updatedForm = await ControlEquipoFrio.findByIdAndUpdate(formId, formData, { new: true });
      ...
      const formIndex = user.controlequipofrio.indexOf(formId);

      // Reemplazar el formulario antiguo con el formulario actualizado
      if (formIndex !== -1) {
        user.controlequipofrio.splice(formIndex, 1, updatedForm._id);
        await user.save();
      }
