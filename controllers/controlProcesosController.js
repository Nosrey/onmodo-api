...
const controlProcesosController = {
  ...
  getFormsByUserIdAndStatus: async (req, res) => {
    try {
      ...
      const forms = await ControlProcesos.find({ idUser: userId, status: { $ne: "" } });
  ...
  editFormProperties: async (req, res) => {
    try {
      ...
      const updatedForm = await ControlProcesos.findByIdAndUpdate(
        formId,
      ...
      const cargaIndex = user.controlproceso.findIndex((c) => c._id.toString() === formId);
      if (cargaIndex !== -1) {
        if (status !== undefined) {
          user.controlproceso[cargaIndex].status = status;
        }
        if (motivo !== undefined) {
          user.controlproceso[cargaIndex].motivo = motivo;
        }
        if (motivoRespuesta !== undefined) {
          user.controlproceso[cargaIndex].motivoRespuesta = motivoRespuesta;
        }
        if (motivoPeticion !== undefined) {
          user.controlproceso[cargaIndex].motivoPeticion = motivoPeticion;
        }
        if (typeof editEnabled === 'boolean') {
          user.controlproceso[cargaIndex].editEnabled = editEnabled;
        }
        if (whoApproved !== undefined) {
          user.controlproceso[cargaIndex].whoApproved = whoApproved;
        }
  ...
  editFormById: async (req, res) => {
    try {
      ...
      const existingForm = await ControlProcesos.findById(formId);
      ...
      const updatedForm = await ControlProcesos.findByIdAndUpdate(formId, formData, { new: true });
      ...
      const formIndex = user.controlproceso.indexOf(formId);

      // Reemplazar el formulario antiguo con el formulario actualizado
      if (formIndex !== -1) {
        user.controlproceso.splice(formIndex, 1, updatedForm._id);
        await user.save();
      }
