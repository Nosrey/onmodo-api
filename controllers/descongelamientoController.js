...
const descongelamientoController = {
  ...
  getFormsByUserIdAndStatus: async (req, res) => {
    try {
      ...
      const forms = await Descongelamiento.find({ idUser: userId, status: { $ne: "" } });
  ...
  editFormProperties: async (req, res) => {
    try {
      ...
      const updatedForm = await Descongelamiento.findByIdAndUpdate(
        formId,
      ...
      const cargaIndex = user.descongelamiento.findIndex((c) => c._id.toString() === formId);
      if (cargaIndex !== -1) {
        if (status !== undefined) {
          user.descongelamiento[cargaIndex].status = status;
        }
        if (motivo !== undefined) {
          user.descongelamiento[cargaIndex].motivo = motivo;
        }
        if (motivoRespuesta !== undefined) {
          user.descongelamiento[cargaIndex].motivoRespuesta = motivoRespuesta;
        }
        if (motivoPeticion !== undefined) {
          user.descongelamiento[cargaIndex].motivoPeticion = motivoPeticion;
        }
        if (typeof editEnabled === 'boolean') {
          user.descongelamiento[cargaIndex].editEnabled = editEnabled;
        }
        if (whoApproved !== undefined) {
          user.descongelamiento[cargaIndex].whoApproved = whoApproved;
        }
  ...
  editFormById: async (req, res) => {
    try {
      ...
      const existingForm = await Descongelamiento.findById(formId);
      ...
      const updatedForm = await Descongelamiento.findByIdAndUpdate(formId, formData, { new: true });
      ...
      const formIndex = user.descongelamiento.indexOf(formId);

      // Reemplazar el formulario antiguo con el formulario actualizado
      if (formIndex !== -1) {
        user.descongelamiento.splice(formIndex, 1, updatedForm._id);
        await user.save();
      }
