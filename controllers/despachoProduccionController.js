    ...
    deleteForm: async (req, res) => {
      ...
      getFormsByUserIdAndStatus: async (req, res) => {
        try {
          ...
          const forms = await DespachoProduccion.find({ idUser: userId, status: { $ne: "" } });
      ...
      editFormProperties: async (req, res) => {
        try {
          ...
          const updatedForm = await DespachoProduccion.findByIdAndUpdate(
            formId,
          ...
          const cargaIndex = user.despachoproduccion.findIndex((c) => c._id.toString() === formId);
          if (cargaIndex !== -1) {
            if (status !== undefined) {
              user.despachoproduccion[cargaIndex].status = status;
            }
            if (motivo !== undefined) {
              user.despachoproduccion[cargaIndex].motivo = motivo;
            }
            if (motivoRespuesta !== undefined) {
              user.despachoproduccion[cargaIndex].motivoRespuesta = motivoRespuesta;
            }
            if (motivoPeticion !== undefined) {
              user.despachoproduccion[cargaIndex].motivoPeticion = motivoPeticion;
            }
            if (typeof editEnabled === 'boolean') {
              user.despachoproduccion[cargaIndex].editEnabled = editEnabled;
            }
            if (whoApproved !== undefined) {
              user.despachoproduccion[cargaIndex].whoApproved = whoApproved;
            }
      ...
      editFormById: async (req, res) => {
        try {
          ...
          const existingForm = await DespachoProduccion.findById(formId);
          ...
          const updatedForm = await DespachoProduccion.findByIdAndUpdate(formId, formData, { new: true });
          ...
          const formIndex = user.despachoproduccion.indexOf(formId);
    
          // Reemplazar el formulario antiguo con el formulario actualizado
          if (formIndex !== -1) {
            user.despachoproduccion.splice(formIndex, 1, updatedForm._id);
            await user.save();
          }
