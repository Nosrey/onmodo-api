...
const distribucionController = {
  ...
  getFormsByUserIdAndStatus: async (req, res) => {
    try {
      ...
      const forms = await Distribucion.find({ idUser: userId, status: { $ne: "" } });
  ...
  editFormProperties: async (req, res) => {
    try {
      ...
      const updatedForm = await Distribucion.findByIdAndUpdate(
        formId,
      ...
      const cargaIndex = user.distribucion.findIndex((c) => c._id.toString() === formId);
      if (cargaIndex !== -1) {
        if (status !== undefined) {
          user.distribucion[cargaIndex].status = status;
        }
        if (motivo !== undefined) {
          user.distribucion[cargaIndex].motivo = motivo;
        }
        if (motivoRespuesta !== undefined) {
          user.distribucion[cargaIndex].motivoRespuesta = motivoRespuesta;
        }
        if (motivoPeticion !== undefined) {
          user.distribucion[cargaIndex].motivoPeticion = motivoPeticion;
        }
        if (typeof editEnabled === 'boolean') {
          user.distribucion[cargaIndex].editEnabled = editEnabled;
        }
        if (whoApproved !== undefined) {
          user.distribucion[cargaIndex].whoApproved = whoApproved;
        }
  ...
  editFormById: async (req, res) => {
    try {
      ...
      const existingForm = await Distribucion.findById(formId);
      ...
      const updatedForm = await Distribucion.findByIdAndUpdate(formId, formData, { new: true });
      ...
      const formIndex = user.distribucion.indexOf(formId);

      // Reemplazar el formulario antiguo con el formulario actualizado
      if (formIndex !== -1) {
        user.distribucion.splice(formIndex, 1, updatedForm._id);
        await user.save();
      }
