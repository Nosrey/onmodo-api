    ...
    deleteForm: async (req, res) => {
      ...
      getFormsByUserIdAndStatus: async (req, res) => {
        try {
          ...
          const forms = await EntregaRopa.find({ idUser: userId, status: { $ne: "" } });
      ...
      editFormProperties: async (req, res) => {
        try {
          ...
          const updatedForm = await EntregaRopa.findByIdAndUpdate(
            formId,
          ...
          const cargaIndex = user.entregaropa.findIndex((c) => c._id.toString() === formId);
          if (cargaIndex !== -1) {
            if (status !== undefined) {
              user.entregaropa[cargaIndex].status = status;
            }
            if (motivo !== undefined) {
              user.entregaropa[cargaIndex].motivo = motivo;
            }
            if (motivoRespuesta !== undefined) {
              user.entregaropa[cargaIndex].motivoRespuesta = motivoRespuesta;
            }
            if (motivoPeticion !== undefined) {
              user.entregaropa[cargaIndex].motivoPeticion = motivoPeticion;
            }
            if (typeof editEnabled === 'boolean') {
              user.entregaropa[cargaIndex].editEnabled = editEnabled;
            }
            if (whoApproved !== undefined) {
              user.entregaropa[cargaIndex].whoApproved = whoApproved;
            }
