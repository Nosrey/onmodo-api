...
const recuperacionProductoController = {
  ...
  getFormsByUserIdAndStatus: async (req, res) => {
    try {
      ...
      const forms = await RecuperacionProducto.find({ idUser: userId, status: { $ne: "" } });
  ...
  editFormProperties: async (req, res) => {
    try {
      ...
      const updatedForm = await RecuperacionProducto.findByIdAndUpdate(
        formId,
      ...
      const cargaIndex = user.recuperacionproducto.findIndex((c) => c._id.toString() === formId);
      if (cargaIndex !== -1) {
        if (status !== undefined) {
          user.recuperacionproducto[cargaIndex].status = status;
        }
        if (motivo !== undefined) {
          user.recuperacionproducto[cargaIndex].motivo = motivo;
        }
        if (motivoRespuesta !== undefined) {
          user.recuperacionproducto[cargaIndex].motivoRespuesta = motivoRespuesta;
        }
        if (motivoPeticion !== undefined) {
          user.recuperacionproducto[cargaIndex].motivoPeticion = motivoPeticion;
        }
        if (typeof editEnabled === 'boolean') {
          user.recuperacionproducto[cargaIndex].editEnabled = editEnabled;
        }
        if (whoApproved !== undefined) {
          user.recuperacionproducto[cargaIndex].whoApproved = whoApproved;
        }
  ...
  editFormById: async (req, res) => {
    try {
      ...
      const existingForm = await RecuperacionProducto.findById(formId);
      ...
      const updatedForm = await RecuperacionProducto.findByIdAndUpdate(formId, formData, { new: true });
      ...
      const formIndex = user.recuperacionproducto.indexOf(formId);

      // Reemplazar el formulario antiguo con el formulario actualizado
      if (formIndex !== -1) {
        user.recuperacionproducto.splice(formIndex, 1, updatedForm._id);
        await user.save();
      }
