    ...
    deleteForm: async (req, res) => {
      ...
      getFormsByUserIdAndStatus: async (req, res) => {
        try {
          ...
          const forms = await SaludManipuladores.find({ idUser: userId, status: { $ne: "" } });
      ...
      editFormProperties: async (req, res) => {
        try {
          ...
          const updatedForm = await SaludManipuladores.findByIdAndUpdate(
            formId,
          ...
          const cargaIndex = user.saludmanipuladores.findIndex((c) => c._id.toString() === formId);
          if (cargaIndex !== -1) {
            if (status !== undefined) {
              user.saludmanipuladores[cargaIndex].status = status;
            }
            if (motivo !== undefined) {
              user.saludmanipuladores[cargaIndex].motivo = motivo;
            }
            if (motivoRespuesta !== undefined) {
              user.saludmanipuladores[cargaIndex].motivoRespuesta = motivoRespuesta;
            }
            if (motivoPeticion !== undefined) {
              user.saludmanipuladores[cargaIndex].motivoPeticion = motivoPeticion;
            }
            if (typeof editEnabled === 'boolean') {
              user.saludmanipuladores[cargaIndex].editEnabled = editEnabled;
            }
            if (whoApproved !== undefined) {
              user.saludmanipuladores[cargaIndex].whoApproved = whoApproved;
            }
      ...
      editFormById: async (req, res) => {
        try {
          ...
          const existingForm = await SaludManipuladores.findById(formId);
          ...
          const updatedForm = await SaludManipuladores.findByIdAndUpdate(formId, formData, { new: true });
          ...
          const formIndex = user.saludmanipuladores.indexOf(formId);
    
          // Reemplazar el formulario antiguo con el formulario actualizado
          if (formIndex !== -1) {
            user.saludmanipuladores.splice(formIndex, 1, updatedForm._id);
            await user.save();
          }
