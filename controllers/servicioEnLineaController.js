...
const servicioEnLineaController = {
  ...
  getFormsByUserIdAndStatus: async (req, res) => {
    try {
      ...
      const forms = await ServicioEnLinea.find({ idUser: userId, status: { $ne: "" } });
  ...
  editFormProperties: async (req, res) => {
    try {
      ...
      const updatedForm = await ServicioEnLinea.findByIdAndUpdate(
        formId,
      ...
      const cargaIndex = user.servicioenlinea.findIndex((c) => c._id.toString() === formId);
      if (cargaIndex !== -1) {
        if (status !== undefined) {
          user.servicioenlinea[cargaIndex].status = status;
        }
        if (motivo !== undefined) {
          user.servicioenlinea[cargaIndex].motivo = motivo;
        }
        if (motivoRespuesta !== undefined) {
          user.servicioenlinea[cargaIndex].motivoRespuesta = motivoRespuesta;
        }
        if (motivoPeticion !== undefined) {
          user.servicioenlinea[cargaIndex].motivoPeticion = motivoPeticion;
        }
        if (typeof editEnabled === 'boolean') {
          user.servicioenlinea[cargaIndex].editEnabled = editEnabled;
        }
        if (whoApproved !== undefined) {
          user.servicioenlinea[cargaIndex].whoApproved = whoApproved;
        }
  ...
  editFormById: async (req, res) => {
    try {
      ...
      const existingForm = await ServicioEnLinea.findById(formId);
      ...
      const updatedForm = await ServicioEnLinea.findByIdAndUpdate(formId, formData, { new: true });
      ...
      const formIndex = user.servicioenlinea.indexOf(formId);

      // Reemplazar el formulario antiguo con el formulario actualizado
      if (formIndex !== -1) {
        user.servicioenlinea.splice(formIndex, 1, updatedForm._id);
        await user.save();
      }
