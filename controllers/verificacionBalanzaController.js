    ...
    deleteForm: async (req, res) => {
      ...
      getFormsByUserIdAndStatus: async (req, res) => {
        try {
          ...
          const forms = await VerificacionBalanza.find({ idUser: userId, status: { $ne: "" } });
      ...
      editFormProperties: async (req, res) => {
        try {
          ...
          const updatedForm = await VerificacionBalanza.findByIdAndUpdate(
            formId,
          ...
          const cargaIndex = user.verificacionbalanza.findIndex((c) => c._id.toString() === formId);
          if (cargaIndex !== -1) {
            if (status !== undefined) {
              user.verificacionbalanza[cargaIndex].status = status;
            }
            if (motivo !== undefined) {
              user.verificacionbalanza[cargaIndex].motivo = motivo;
            }
            if (motivoRespuesta !== undefined) {
              user.verificacionbalanza[cargaIndex].motivoRespuesta = motivoRespuesta;
            }
            if (motivoPeticion !== undefined) {
              user.verificacionbalanza[cargaIndex].motivoPeticion = motivoPeticion;
            }
            if (typeof editEnabled === 'boolean') {
              user.verificacionbalanza[cargaIndex].editEnabled = editEnabled;
            }
            if (whoApproved !== undefined) {
              user.verificacionbalanza[cargaIndex].whoApproved = whoApproved;
            }
      ...
      editFormById: async (req, res) => {
        try {
          ...
          const existingForm = await VerificacionBalanza.findById(formId);
          ...
          const updatedForm = await VerificacionBalanza.findByIdAndUpdate(formId, formData, { new: true });
          ...
          const formIndex = user.verificacionbalanza.indexOf(formId);
    
          // Reemplazar el formulario antiguo con el formulario actualizado
          if (formIndex !== -1) {
            user.verificacionbalanza.splice(formIndex, 1, updatedForm._id);
            await user.save();
          }
